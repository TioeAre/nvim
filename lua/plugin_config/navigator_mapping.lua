...
function M.setup(attach_opts)
    ...
    if _NgConfigValues.lsp.call_hierarchy.enable and cap.call_hierarchy or cap.callHierarchyProvider then
    ...
    if _NgConfigValues.lsp.definition.enable then
        ...
        for _, value in pairs(key_maps) do
            if value.func == vim.lsp.buf.definition or value.func == vim.lsp.buf.type_definition then
                vim.keymap.del(value.mode or 'n', value.key, {
                    buffer = attach_opts.bufnr or vim.api.nvim_get_current_buf()
                })
            end
        end
    end
