...
local open_file = function(filename, lnum, col, winid)
    ...
    vim.cmd("e " .. vim.fn.fnameescape(filename)) -- for vertical split
    -- vim.cmd("wincmd l")
...
local render_file = function(current_item, if_highlight_current)
    if not current_item.is_disable then
        ...
        if config.lnum_for_results == true then
            item_line_len = string.len(current_item.item.lnum) + 1
        end
        ...
        if if_highlight_current then
            ...
            if state.last_select_item and current_item.c_line ~= state.last_select_item.c_line then
