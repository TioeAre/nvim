...
local lsp_vars = require("variables.lsp")
...
M.config_lspconfig = function()
	...
	local installed_servers = vim.tbl_keys(lsp_vars.user_lspconfig_servers)
	...
	-- neovim/nvim-lspconfig
	for server, config in pairs(lsp_vars.user_lspconfig_servers) do
	...
	require("navigator").setup({
		ts_fold = {
			...
			disable_filetypes = filetype.excluded_filetypes,
		},
		icons = {
			icons = true,
			code_action_icon = "󰌵",
		...
		lsp = {
			...
			servers = vim.tbl_keys(lsp_vars.navigator_servers),
			hover = {
				enable = false,
			},
		},
	...
	for type, icon in pairs(lsp_vars.diagnostic_signs) do
	...
	-- call
	lsp_utils.show_only_one_sign_in_sign_column()
...
M.config_symbol_usage = function()
	require("symbol-usage").setup({
		text_format = lsp_utils.text_format,
	})
end
...
M.config_lsp_progress = function()
	require("lsp-progress").setup({
		client_format = lsp_utils.client_format,
		format = lsp_utils.format,
	})
